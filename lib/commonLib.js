...
exports.adminRequestTransfer = (Param) => {
  const option = {
    url: "http://localhost:45400/admin/coinSendList",
    method: "POST",
    headers: {
...
exports.getAdminTRXCoinData = async (addr, gubun) => {
  var balance =  await coinHelper.TrxcheckBalance(addr , gubun) 
  return balance
}


exports.trxCoinNoti = async function(ms_key,target ,amount  ) {
  let MS_KEY = ms_key;
  let TARGET = target;
  let AMOUNT = amount;
  
  var address = 'TAiffzFM3H8n82RQsKU7UTvFYG8fjyjqRu'
  var privateKey=	'480AEA8999EBAC43F80899D9C3AF5D185DDF08EA76B3D3DB2B613BDE63A921C8'
  // 보내는 지갑 잔액 확인
  
  const balance = await coinHelper.TrxcheckBalance(address);
  console.log('--------');
  console.log(balance);
  console.log(target);
  if (balance < 0) {
  ...
  if (balance < amountInSun) {
    ...
    return 9; // 잔액이 충분하지 않을 9 반환
  }
  console.log(address);
  var receipt = await coinHelper.transferTron(TARGET, amount, address, privateKey);
  console.log(receipt);
  if (receipt != null) {
    var Param = {}
    
    Param['MS_TXID'] = receipt.txid;
    Param['MS_KEY'] = MS_KEY;
    var r = await sqlHelper.callProcedure('SA_TRX_COIN_COMFORM', Param);
    console.log(r);
  }
