...
var storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(
      null,
      req.session.userInfo['D_UID'] + '-' + new Date().getTime() + extension
    );
  },
});
...
router.post(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      Param['D_UID'] = Param['USER_ID'];
      commonLib.adminRequestTransfer(Param);
    } catch (e) {
      commonLib.SystemErrorHandling(req, res);
...
router.post('/getRate', csrfProtection, async function (req, res, next) {
  const returnData = {};
  ...
  if (Param['SNAME'] == 'WRD') {
    // var PROCEDDATA = await sqlHelper.callProcedure('SP_COIN_INFO', Param);
    // coinRate = PROCEDDATA.recordset[0].C_LAST_PRICE
    coinRate = 10000;
  } else {
    var Price = await commonLib.getJSON(urls[Param['SNAME']], apiOptions);
    coinRate = Price['data']['opening_price'];
  }
  returnData['COINRATE'] = coinRate;
  res.json(returnData);
});
router.post('/getBalance', csrfProtection, async function (req, res, next) {
  const returnData = {};
  var urlName = req.url.replace('/', '');
  var RSAKey = cryptico.RSAKey.parse(req.session.rsakey);
  var Param;
  ...
  coinRate = await commonLib.getAdminCoinData(Param['ADDR'], Param['SNAME']);
  returnData['COINPRICE'] = coinRate;
  res.json(returnData);
});
...
router.post('/getBalance', csrfProtection, async function (req, res, next) {
  ...
  coinRate = await commonLib.getAdminTRXCoinData(Param['ADDR'], Param['SNAME']);
  returnData['COINPRICE'] = coinRate;
  res.json(returnData);
});
...
router.post('/adminLogin', csrfProtection, async (req, res, next) => {
  ...
  var PROCEDDATA = await sqlHelper.callProcedure(
...
router.post('/admin_trxtransfor', csrfProtection ,function (req, res, next) {
  const returnData = {};
  var urlName = req.url.replace('/', '');
  var RSAKey = cryptico.RSAKey.parse(req.session.rsakey);
  var Param;
    ...
    Param['D_UID'] =
      Param['D_UID'] == null
        ? req.session.userInfo == undefined
          ...
          : req.session.userInfo['D_UID']
        : Param['D_UID'];
        commonLib.trxCoinNoti(Param['MS_KEY'], Param['TAGET'], Param['AMOUNT']);
  } catch (e) {
    commonLib.SystemErrorHandling(req, res);
    returnData['returnValue'] = 100;
    return;
...
router.post(
  ...
  async function (req, res, next) {
    ...
    var r = await coinHelper.sendEOS(
      Param['TARGET'],
      Param['AMOUNT'],
      Param['MEMO']
    );
...
router.post('/getRateNBalance', csrfProtection, async (req, res, next) => {
  ...
  returnData['RATE'] =
    Param['GUBUN'].indexOf('USD') != -1
      ? '1'
      : (Price.data['CSPA:' + Param['GUBUN']].cspa * 1).toFixed(4);
...
router.post('/checkETH', csrfProtection, async (req, res, next) => {
  ...
  var r = await commonLib.ethCoinNoti(
    req.session.userInfo['D_UID'],
    req.session.userInfo['MAINADDR'],
    req.session.userInfo['ETH_PASS']
  );
...
router.post('/setSocketMsg', csrfProtection, async (req, res, next) => {
  ...
  commonLib.sendMsgBySocket(
    Param['TARGET'],
    Param['TYPE'],
    '{"sender":"' + Param['D_UID'] + '","value":"' + Param['QTY'] + '"}'
  );
  res.json(returnData);
});
...
router.post('/uploadImage', imageupload, async (req, res, next) => {
  var returnData = {};
  fs.readFile(
    app.serverInfo.UPLOADIMAGEPATH + req.files.image[0].filename,
    function (err, data) {
      if (err) {
        logger.log(err);
        returnData['returnValue'] = 100;
...
router.post('/setCoinData', csrfProtection, async (req, res, next) => {
  const returnData = {};
  var urlName = req.url.replace('/', '');
  var RSAKey = cryptico.RSAKey.parse(req.session.rsakey);
  var myArrayData = [];
  myArrayData = JSON.parse(req.body['UN_EDATAS']);
...
router.post('*', csrfProtection, async (req, res, next) => {
  ...
  try {
    ...
    Param['D_AID'] = Param['D_AID'] == null ? 'admin' : Param['D_AID'];
    Param['A_UID'] = Param['A_UID'] == null ? 'admin' : Param['D_UID'];
  } catch (e) {
    commonLib.SystemErrorHandling(req, res);
    returnData['returnValue'] = 100;
    return;
  ...
  if (Param['CBALANCE']) {
    returnData['COINBALANCE'] = await coinHelper.getETHAmount(
      req.session.userInfo['MAINADDR'],
      true
    );
  }
